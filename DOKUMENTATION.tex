...
\begin{itemize}
  \item \textbf{Docker} und \textbf{Docker Compose} (für MongoDB und Neo4j)
  \item \textbf{Node.js} (v18+) und \textbf{npm} (für Backend und Frontend)
\end{itemize}

\subsection{Schritt 1: Repository klonen}
...
\subsection{Schritt 2: Datenbanken mit Docker starten}
...
\subsection{Schritt 3: Backend einrichten}
...
\texttt{.env}-Datei im \texttt{backend/}-Ordner erstellen:
...
\subsection{Schritt 4: Datenbank mit Testdaten befüllen}

\begin{lstlisting}[language=bash]
node src/seed.js
\end{lstlisting}

\subsection{Schritt 5: Backend starten}

\begin{lstlisting}[language=bash]
npm run dev
\end{lstlisting}

\subsection{Schritt 6: Frontend starten (neues Terminal)}
