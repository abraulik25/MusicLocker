...
\begin{enumerate}
  \item \textbf{Frontend $\rightarrow$ Backend}: React-Komponenten rufen REST-Endpunkte auf (\texttt{/api/...}). Die Authentifizierung erfolgt über JWT-Tokens im \texttt{Authorization}-Header. Eine vorgeschaltete \textbf{JWT-Middleware} im Backend prüft diese Tokens auf Gültigkeit, bevor die Anfrage an die eigentlichen Routen weitergeleitet wird.
...
MongoDB fungiert als Hauptspeicher für alle Stammdaten. Da es schemafrei ist, können Dokumente flexibel erweitert werden. Dennoch wird durch Validierungslogik im Backend eine konsistente Struktur (Schema Validation) erzwungen.

\subsubsection{Collections}

Die Datenbank \texttt{musikempfehlung} enthält folgende Haupt-Collections:

\begin{itemize}
  \item \textbf{users}: Beinhaltet Benutzerprofile, Rollen, gehashte Passwörter und Präferenzen.
  \item \textbf{artists}: Speichert Informationen zu Musikern und Bands.
  \item \textbf{albums}: Verwaltet Alben, deren Veröffentlichungsjahr und Track-Anzahl.
  \item \textbf{tracks}: Enthält einzelne Songs, deren Metadaten und zugeordnete Stimmungen (Moods).
  \item \textbf{playlists}: Protokolliert von Benutzern erstellte Song-Sammlungen.
  \item \textbf{moods}: Beschreibt die im System verfügbaren Stimmungen für das Empfehlungssystem.
\end{itemize}

\subsubsection{Schema-Definition und Beispiele}

Nachfolgend sind die JSON-Schemata für die wichtigsten Entitäten dargestellt.

\textbf{1. User (Benutzer)} Ein User repräsentiert eine Person, die das System nutzt.
...
\textbf{2. Artist (Künstler)} Ein Artist repräsentiert einen Musiker oder eine Band.
...
\textbf{3. Album} Ein Album gruppiert mehrere Tracks eines Künstlers.
...
\textbf{4. Track (Song)} Ein Track ist ein einzelnes Musikstück mit zugeordneten Moods.
...
\textbf{5. Playlist} Eine Playlist ist eine vom Benutzer erstellte Sammlung von Tracks.
...
\textbf{6. Mood (Stimmung)} Ein Mood fungiert als Tag für das Empfehlungssystem.
